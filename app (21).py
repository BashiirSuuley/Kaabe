...
if tool == "slides":
    ...
    if st.button("✨ " + ("توليد الشرائح" if ar_only else "Generate Slides"), type="primary", use_container_width=True):
        ...
        try:
            ...
            prompt = f"""You are a world-class university professor preparing a professional academic presentation.
Your job: read the uploaded PDF with FULL academic depth and create a {num_slides}-slide presentation.
{ci} {fi}

{lang_note}

═══ EXTRACTION RULES (follow every one) ═══
1. Read the ENTIRE document carefully — every section, every page.
2. Extract EXACT content: real names, real numbers, real definitions, real formulas.
3. NEVER say "from the document", "the text states", "as mentioned" — state facts directly as a professor would.
4. NEVER invent, assume, or add content not in the PDF.
5. Every bullet point must contain SPECIFIC information — not vague generalities.
6. Extract EVERY formula, equation, and mathematical expression EXACTLY as written.
7. For every formula: copy it exactly, define every variable, provide a real worked numerical example with full step-by-step calculation.
8. Extract ALL tables with exact headers and all data rows.
9. Identify the key topics, subtopics, definitions, examples, and conclusions.
10. Structure slides like a real university lecture: introduce → explain → demonstrate → conclude.

═══ SLIDE STRUCTURE RULES ═══
- Slide 1: ALWAYS type "title" — title, subtitle, author, course name
- Slides 2-3: Overview and introduction bullets — real key facts from the document
- Middle slides: Mix of lecture/formula/two_col/table/stat_callout based on actual content
- Use "lecture" type for any concept that has theory + formula + example
- Use "formula" type for standalone equations with worked examples
- Use "table" type for ANY tabular data found in the document
- Use "two_col" for comparisons, classifications, before/after, pros/cons
- Use "stat_callout" for slides dominated by key statistics or numbers
- Last slide: ALWAYS type "conclusion" — real key takeaways from the document
- Vary slide types — never use same type more than 2 times in a row

═══ QUALITY REQUIREMENTS ═══
{"- FORMULAS: Extract every formula exactly as written. Include: variable definitions, units, worked numerical example, step-by-step solution." if incl_f else ""}
{"- EXAMPLES: Every formula/concept must have a real worked example from the document with full numeric calculation." if incl_e else ""}
{"- TABLES: Every table in the PDF must become a table slide with exact headers and all data rows." if incl_t else ""}

Return raw valid JSON only. No markdown fences. No extra text. Strings must be on ONE line.
For calculations with multiple steps use \n between steps.

JSON FORMAT (follow exactly):
{{
  "title": "exact document title",
  "subtitle": "subject or course name",
  "author": "{author_lbl}",
  "slides": [
    {{
      "slide_num": 1,
    ...
    {{
      ...
      "title": "slide title",
      "bullets": [
        "Specific real fact 1 with exact data from document",
        "Specific real fact 2 with exact data",
        "Specific real fact 3 with exact data",
        "Specific real fact 4 with exact data",
        "Specific real fact 5 with exact data"
      ]
    }},
    {{
      "slide_num": 3,
      "type": "lecture",
      "title": "concept name from document",
      "theory": "Full clear academic explanation of this concept as a professor would explain it",
      "formula": "exact formula e.g. P(A) = n(A) / n(S)",
      "where": "P(A) = probability of event A, n(A) = number of favourable outcomes, n(S) = total sample space",
      "example": "Exact numerical example: A bag has 3 red and 7 blue balls. Find P(red).",
      "calculation": "P(red) = n(red) / n(total)\nP(red) = 3 / 10\nP(red) = 0.3 = 30%"
    }},
    {{
      "slide_num": 4,
      "type": "formula",
      "title": "formula name from document",
      "formula": "exact formula from document",
      "where": "var1 = description with units, var2 = description with units",
      "example": "Real numerical example from document",
    ...
    {{
      ...
      "left_points": ["specific point 1", "specific point 2", "specific point 3", "specific point 4"],
      "right_title": "right category",
      "right_points": ["specific point A", "specific point B", "specific point C", "specific point D"]
    }},
    {{
      "slide_num": 6,
      "type": "stat_callout",
      ...
      "stats": [
        {{"value": "exact number", "label": "what it measures"}},
        {{"value": "exact number", "label": "what it measures"}},
        {{"value": "exact number", "label": "what it measures"}}
      ],
      "body": "context explaining these statistics"
    }},
    {{
      "slide_num": 7,
      "type": "table",
    ...
    {{
      ...
      "title": "{conc_title}",
      "bullets": [
        "Main finding 1 — specific and detailed",
        "Main finding 2 — specific and detailed",
        "Key formula learned and its application",
        "Practical application or real-world use",
        "Next topics or further study direction"
      ]
    }}
  ]
}}

IMPORTANT: Generate exactly {num_slides} slides total.
Cover ALL major topics in the document.
Make every slide CONTENT-RICH with real specific information.
A university lecturer would be proud to use these slides in a real class."""
